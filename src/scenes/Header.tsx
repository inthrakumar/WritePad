  ...
  return (
    <div className="max-w-10xl w-[100vw] font-poppins flex justify-between items-center fixed top-0  max-h-[75px] shadow-sm bg-white ">
      ...
      <div className="flex text-lg items-center justify-center gap-5 mr-8">
        <Link href={'/documents'}>
          <span className="tracking-tight hover:text-orange-700 ">My Docs</span>
        </Link>
        <Link href={'/shared-docs'}>
          <span className="tracking-tight hover:text-orange-700 ">Shared</span>
        </Link>

        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
              <Avatar onClick={handleUserProfileRedirect}>
                {user.isSignedIn ? (
                  <AvatarImage
        ...
        <div>
          ...
          <AuthLoading>
            <Button>
              <RiLoader5Line size={30} className="animate-spin" />
            </Button>
          </AuthLoading>
        </div>
      </div>
